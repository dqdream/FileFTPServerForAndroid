...
public class CmdCWD extends FtpCmd implements Runnable {
    ...
    public void run() {
        Log.d(TAG, "run.");
        myLog.l(Log.DEBUG, "CWD executing"+input);
...
//            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            newDir = new File(Environment.getExternalStorageDirectory(), param);

//            } else {
//                newDir = inputPathToChrootedFile(sessionThread.getWorkingDir(), param);
//            }

            LogUtil.d("ftp_vv", Build.VERSION.SDK_INT + " - " + newDir.getAbsolutePath() + "---," + sessionThread.getWorkingDir().getAbsolutePath() + "," + param);
