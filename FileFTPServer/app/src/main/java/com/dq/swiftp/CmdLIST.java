...
public class CmdLIST extends CmdAbstractListing implements Runnable {
    ...
    protected String makeLsString(File file) {
        ...
        if(file.getAbsolutePath().equals(DefaultStorageManager.getInstance().getStorageMountPath(DefaultStorageManager.TYPE_INTERNAL))) {
            response.append(Defaults.STR_INTERNAL);
        } else if(file.getAbsolutePath().equals(DefaultStorageManager.getInstance().getStorageMountPath(DefaultStorageManager.TYPE_SDCARD))) {
            response.append(Defaults.STR_EXTERNAL);
        } else {
