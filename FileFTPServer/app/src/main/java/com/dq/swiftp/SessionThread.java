...
public class SessionThread extends Thread {
    ...
    public boolean sendViaDataSocket(String string) {
        Log.d(TAG, "sendViaDataSocket, string: " + string);
        try {
    ...
    public int receiveFromDataSocket(byte[] buf) {
        Log.d(TAG, "receiveFromDataSocket.");
        int bytesRead;
    ...
    public boolean startUsingDataSocket() {
        Log.d(TAG, "startUsingDataSocket.");
        try {
    ...
    public void run() {
        ...
        try {
            BufferedReader in = new BufferedReader(new InputStreamReader(cmdSocket
                    .getInputStream(), Charset.defaultCharset()), 8192); // use 8k buffer
            while (true) {
                String line;
    ...
    public void writeBytes(byte[] bytes) {
        try {
            ...
            BufferedOutputStream out = new BufferedOutputStream(cmdSocket
                    .getOutputStream(), Defaults.getDataChunkSize());
            out.write(bytes);
            out.flush();
    ...
    public void writeString(String str) {
        ...
        try {
            ...
            strBytes = str.getBytes(Charset.defaultCharset());
        }
        writeBytes(strBytes);
    }
    ...
    public void authAttempt(boolean authenticated) {
        Log.d(TAG, "authAttempt, authenticated: " + authenticated);
        if (authenticated) {
